...
> **Про фото саме в цій таблиці.** Перевірено на живих даних: у таблиці немає
> формул `=IMAGE()` — фото вставлені як **об'єкти в клітинки**. Отримати їх
> можна лише експортом `.xlsx`. Публічний експорт
> `https://docs.google.com/spreadsheets/d/<ID>/export?format=xlsx` не потребує
> ключа й спрацьовує на звичайному хостингу (Vercel тощо) — тоді фото
> підтягнуться автоматично. Якщо ж середовище блокує `docs.google.com`
> (напр. корпоративний файрвол), задайте `GOOGLE_SERVICE_ACCOUNT_JSON` з
> увімкненим Drive API. Поки фото недоступні — показується плейсхолдер, решта
> каталогу (ціни, розміри, наявність, сітка) працює повністю.

    ...
    input_schema: {
      ...
      properties: {
        query: {
          type: 'string',
          description:
            'ТОЛЬКО бренд + модель/тип, как ты определил по фото (напр. «Nike Mercurial Vapor», «Adidas Predator»). ' +
  ...
  try {
    switch (name) {
      case 'match_photo': {
        ...
        const found = (await searchProducts(query, 60, { relaxed: true }))
        ...
        const blocks: (Anthropic.TextBlockParam | Anthropic.ImageBlockParam)[] = [
          {
            type: 'text',
            text:
              `Ниже ВСЕ расцветки этой модели из каталога (${found.length} шт.), которые есть в наличии. ` +
              'Внимательно просмотри КАЖДУЮ и сравни с фото клиента по цвету. Расцветка есть только на картинке — не в названии, ' +
              'поэтому нельзя судить по тексту, надо смотреть глазами. ' +
              'Если нашёл совпадение по расцветке (пусть даже близкое) — покажи именно её через send_product_card и переходи к оформлению. ' +
              'НЕ говори «нет в наличии», пока не просмотрел все карточки ниже. Если точь-в-точь такой расцветки правда нет среди них — ' +
